...
import { LogOut, Wallet, Briefcase, Repeat, Menu, X, Plus, Handshake } from "lucide-react";
...
useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [walletRes, skillsRes, txnRes, reqRes] = await Promise.all([
          ...
          api.get("/skills/requests/my"),
        ]);
  
        setWallet(walletRes.data.balance);
  
        // Robust filter: handle offeredBy being either a string id or a populated object
        const allSkills = Array.isArray(skillsRes.data) ? skillsRes.data : [];
        const mySkills = allSkills.filter((s) => {
          const offeredBy = s.offeredBy;
          if (!offeredBy || !user) return false;
  
          // If offeredBy is a simple string (id or email)
          ...
          // If offeredBy is an object (populated user)
          if (typeof offeredBy === "object") {
            return (
              offeredBy._id === user.id ||
              offeredBy._id === user._id ||
              offeredBy.id === user.id ||
              offeredBy.email === user.email
            );
          }
        ...
        setSkills(mySkills); // <-- only set the current user's skills here
    ...
    // If user is not available yet, don't fetch my-skills (prevents showing others)
    if (!user) {
      setSkills([]);
      setTransactions([]);
      setRequests([]);
    ...
    fetchData();
  }, [user]); // re-run when logged-in user changes
  ...
  const handleCompleteRequest = async (id) => {
    try {
      await api.patch(`/skills/request/${id}/complete`);
                      ...
                      >
                        <div>
                          <p className="font-medium text-gray-200">
                            {r.skill?.title} —{" "}
                            <span className="text-orange-300">{r.status}</span>
                          </p>
                          <p className="text-sm text-gray-400">
                            Requested by {r.requester?.name} | Provider:{" "}
                            {r.provider?.name}
                          </p>
                        </div>

                        {r.provider?._id === user._id && r.status === "Pending" && (
                          <button
                  ...
                  <div className="space-y-4">
                    {transactions.map((txn) => (
                      ...
                      >
                        <div>
                          <p className="font-medium text-gray-200">
                            {txn.skill?.title || txn.skill}
                          </p>
                          <p className="text-sm text-gray-500">
                            {txn.buyer === user.email
                              ? `You purchased from ${txn.seller}`
                              : `You sold to ${txn.buyer}`}
                          </p>
                        </div>
