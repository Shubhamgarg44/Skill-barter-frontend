 ...
 useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (typeof toast !== "undefined") toast.error("Failed to load dashboard data.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="space-y-4">
        {requests
          .filter((r) => {
            ...
            const now = dayjs();
            if (requestFilter === "day") return date.isAfter(now.startOf("day"));
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Requested by{" "}
                  <span className="text-teal-400">{r.requester?.name}</span> →{" "}
                  Provider:{" "}
                  <span className="text-orange-400">{r.provider?.name}</span>
                </p>
              ...
              {r.requester?._id === user._id && r.status === "Accepted" && (
                <button
