 ...
 useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const reqRes = await api.get("/skills/requests/my");
  
        // ✅ Set wallet, transactions, and requests as before
        setWallet(walletRes.data.balance || 100);
        setTransactions(txnRes.data || []);
        setRequests(reqRes.data || []);
  
        // ✅ Filter my skills safely (handle empty user or unpopulated offeredBy)
        ...
        const filteredSkills = user
          ? allSkills.filter((s) => {
              ...
              // If offeredBy is populated (object)
              ...
              // If offeredBy is just an ID string
              return (
                offeredBy === user._id ||
                offeredBy === user.id ||
    ...
    // ✅ Only run when user exists (prevents undefined crash)
    if (user && user._id) {
      fetchData();
    }
  }, [user]);
